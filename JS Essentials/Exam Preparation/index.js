function getData() {
    ...
    for (let i = 0; i < input.length - 1; i++) {
        ...
        if (command === 'peopleIn'){
            let blacklisted = blacklist.find(p => p.firstName === currentPerson.firstName && p.lastName === currentPerson.lastName);

            if (!blacklisted){
                peopleInList.push(currentPerson);
            }
        }else if (command === 'peopleOut'){
            let personInList = peopleInList.find(p => p.firstName === currentPerson.firstName && p.lastName === currentPerson.lastName);
